...
The controller comparison holds the development records, observable prefix features, four whole-task folds, 200-tree capacity, margin grid, and action menu fixed. One forest predicts arm outcomes. A second predicts signed benefits relative to continuation,
\begin{equation}
\Delta_a(x)=\mathbb{E}[Y_a-Y_0\mid X=x],\qquad a\neq0.
\end{equation}
Both controllers can choose \textsc{Continue}. The candidate set also contains the development-best fixed arm, the strongest runnable policy from the existing arm-outcome family, a comparison-only learner, and a failure-risk detector. All predictions were committed before confirmation arm outcomes were generated.

The confirmation panel contains \AlfPlanned{} ALFWorld tasks balanced over six task types and grouped by \AlfGroups{} floorplans. Their goal directories are absent from the supplied prior configuration and outcome-exposure packet. A secondary panel contains \SwPlanned{} previously reserved ScienceWorld official-test variations that were not present in that packet. The second confirmation adds \EventPlanned{} further identity-disjoint ALFWorld tasks, run under both checkpoint rules. We report the complete primary family, every frozen policy, costs, early-terminal tasks, and the adverse historical evidence.
...
The freeze is what makes the variance reduction safe. Fitting the machinery that lowers variance on the data it will later evaluate is enough to produce any desired conclusion: in extensive-form game evaluation, a heuristic value function tuned on the evaluation sample can be driven to pathologically low variance or to a chosen test statistic, so the function must be fixed before that sample is seen \citep{kim2026aivat}. Our controllers, transforms, and decision rules are hashed and committed before any confirmation outcome exists for the same reason. Panel size follows from the development operating point rather than from convenience. Treating each task as a paired binary comparison \citep{miller2024errorbars}, the development contrast against continuation needs roughly a thousand analysable tasks for conventional power, while the contrast against the best unconditional action needs an order of magnitude more; the second is therefore reported as an estimate and is not tested. Repeated continuations are the variance reduction that makes the first contrast measurable at all, which is the same device \citet{miller2024errorbars} recommends for single-turn evaluations. Intervals are 10,000-resample percentiles at two levels: a task bootstrap over task blocks and a grouped bootstrap that resamples complete floorplans or ScienceWorld families. They are descriptive; the sign-flip and Holm procedure is the decision rule. The evaluator also reports every remaining policy contrast, intervention rate, action counts, recoveries, disruptions, requests, tokens, and client wall time, and every ScienceWorld comparison is retained regardless of sign. No task is removed for a poor outcome or long trajectory, and missing eligible cells block final analysis. Appendix~\ref{app:statistics} gives the complete specification and Appendix~\ref{app:lineage} the administrative resumes. The first study charged 47,876.7 H100 GPU-seconds against a 72,000-second occupancy cap; the second study carries its own ledger.
...
\textbf{Estimation.} The technical comparison isolates two method choices. Outcome modeling estimates every response function; direct signed-benefit modeling estimates effects relative to continuation, as in the response-versus-effect distinction studied by \citet{kunzel2019metalearners}. Cross-draw scoring then separates maximization from evaluation, following the same statistical principle as Double Q-learning \citep{vanhasselt2010double}, and Section~\ref{sec:crossdraw} carries that principle into the controller. Unlike observational treatment-effect estimation, every action here is executed from a serialized simulator state. The estimand is therefore the utility of the frozen action menu, not a general causal effect or an unmatched benchmark rank. Our environments randomize only token sampling. Section~\ref{sec:analysis} shows that the diagnostic is a function of per-task outcome variance, so a domain whose tools and simulated users add randomness of their own, as in $\tau$-bench \citep{yao2024taubench}, moves it upward rather than downward.

\section{Limitations, ethics, and conclusion}

\paragraph{Limitations.} The confirmations cover one 8B actor, two text environments, six fixed repairs, one trigger rule, binary terminal success, and two continuation draws per action. Two draws estimate each action's continuation distribution coarsely. The ALFWorld identities are new relative to the supplied archive rather than absent from pretraining, and the group bootstrap inherits the benchmark's floorplan grouping. Neither panel supports claims about web agents, code agents, safety-critical tools, human interventions, or arbitrary action proposals.
