...
Define $\widehat a_i^{(r)}=\arg\max_a Y_{ia}^{(r)}$ with continuation-first tie breaking. A same-draw analysis scores this outcome-informed choice as $\max_aY_{ia}^{(r)}$. Cross-draw evaluation instead scores the selected action on the other draw. We use the symmetric gap
\begin{equation}
G_i=\frac{1}{2}\!\left[
