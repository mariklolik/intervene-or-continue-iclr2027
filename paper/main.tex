...
\textbf{Proposition 1.} $G_i\geq0$ for every realized pair of action-outcome vectors. If the two draws are identically distributed conditional on $X_i$, then $\mathbb{E}[G_i\mid X_i]$ equals the expected same-draw maximum minus the independent-draw value of the action chosen on that draw.
...
Reasoning-and-acting agents maintain an action history and ground language-model decisions in environment feedback \citep{yao2023react}. Reflexion uses feedback from earlier attempts \citep{shinn2023reflexion}; Self-Refine iterates feedback and generation within a task \citep{madaan2023selfrefine}. These methods alter how an actor recovers, but they do not estimate the value of a fixed intervention menu from matched stochastic continuations.

Runtime supervision now covers several decision types. Calibration Is Not Control defines intervention advantage and evaluates RF-based intervention policies from same-prefix branches \citep{zhang2026calibration}. COTA trains a small pairwise advisor to rank concrete candidate actions \citep{jiang2026cota}. CHILL-Harness learns effects of reasoning procedures in different long-horizon workloads \citep{fu2026chill}. DreamGuard predicts trajectory risk and maps it to pass, hold, or block decisions \citep{lin2026dreamguard}. Harness-R1 edits four lifecycle hooks around a frozen agent, scores candidate patches by fresh task reruns, and separately tests patches inferred from sparse failures on held-out tasks \citep{shao2026harnessr1}. RAIL instead learns a no-branch-gated controller for allocating training rollouts to an evolving policy; its outcome is reward contrast rather than frozen-actor task utility \citep{zhang2026rail}. BOA searches a stochastic trajectory tree for low-probability safety failures, exposing a broader version of the gap between one realized run and a deployment distribution \citep{lin2026safetymeasurement}. These systems establish the importance of acting on or measuring runtime signals. They use different interventions, histories, objectives, and actor budgets, so their headline task scores are not matched baselines for our per-prefix four-arm study.

Our controller comparison follows the outcome-model versus direct-effect distinction in heterogeneous treatment-effect estimation \citep{kunzel2019metalearners}. The repeated branches are not observational causal data: all four actions are executed from a simulator checkpoint, and inference targets this experimental action menu. The design also resembles methods that decouple maximization from evaluation \citep{vanhasselt2010double} and work on adaptive holdout reuse \citep{dwork2015generalization}. Our contribution is the measurement and controlled application to stochastic runtime intervention, not a new general theorem about post-selection inference.
