...
The first confirmation's null controller result bounds low-capacity tabular learners on a coarse menu at an early scheduled step; the headroom reading in Section~\ref{sec:analysis} shows that the ceiling there was set by that design, so it is not evidence that runtime intervention lacks value in general. ScienceWorld is boundary evidence with a low continuation success rate, which compresses both the gap and every contrast; it is reported in full and pooled with nothing.
