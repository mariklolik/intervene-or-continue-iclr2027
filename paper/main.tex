...
The controller comparison holds the development records, observable prefix features, four shuffled task-ID folds, 200-tree capacity, margin grid, and action menu fixed. One forest predicts four arm outcomes. A second predicts three signed benefits relative to continuation,
\begin{equation}
...
Let $X_i$ be the observable prefix for task $i$ at a fixed checkpoint. The action set is $\mathcal{A}=\{0,1,2,3\}$, where $0$ means continue without an injected message. A stochastic continuation under action $a$ produces binary task success $Y_{ia}\in\{0,1\}$ with conditional mean $\mu_a(x)=\mathbb{E}[Y_{ia}\mid X_i=x]$. For a prefix-only policy $\pi(a\mid x)$, the target value is
\begin{equation}
V(\pi)=\mathbb{E}_X\!\left[\sum_{a\in\mathcal{A}}\pi(a\mid X)\mu_a(X)\right].
\label{eq:policy-value}
\end{equation}
The expectation in Eq.~\eqref{eq:policy-value} is over tasks that reach the checkpoint and includes continuation randomness. To match the planned-task success rates reported below, let $E_i$ indicate that task $i$ reaches its assigned checkpoint and let $T_i$ be its observed success if it terminates earlier. The full task-level value is
\begin{equation}
V_{\mathrm{task}}(\pi)=\mathbb{E}\!\left[(1-E_i)T_i+
E_i\sum_{a\in\mathcal A}\pi(a\mid X_i)\mu_a(X_i)\right].
\label{eq:task-value}
\end{equation}
No intervention decision exists when $E_i=0$, so such tasks contribute equally to all policy values and zero to their contrasts. A controller that always intervenes can have lower value than continuation even when it correctly detects a difficult prefix. Detection quality and treatment choice are different estimands \citep{vasudev2026failure}.
...
Define $\widehat a_i^{(r)}=\arg\max_a Y_{ia}^{(r)}$ with deterministic action-index tie order $0\rightarrow1\rightarrow2\rightarrow3$. A same-draw analysis scores this outcome-informed choice as $\max_aY_{ia}^{(r)}$. Cross-draw evaluation instead scores the selected action on the other draw. We use the symmetric gap
\begin{equation}
G_i=\frac{1}{2}\!\left[
...
\textbf{Proposition 1.} $G_i\geq0$ for every realized pair of action-outcome vectors. If the two action-outcome vectors are conditionally independent and identically distributed given $X_i$, then $\mathbb{E}[G_i\mid X_i]$ equals the expected same-draw maximum minus the independent-draw value of the action chosen on that draw.

The pathwise statement follows because each maximum is at least the outcome of the action selected on the other draw. Conditional exchangeability supplies the expectation identity. Independence is needed to interpret the cross-draw score as fresh policy evaluation, but not for nonnegativity. A positive $G_i$ can arise from unstable binary outcomes even when all actions have the same conditional mean. We therefore treat $G$ as an evaluation diagnostic, not as treatment benefit, action heterogeneity, or controller regret. Because $G_i\geq0$ by construction, sign-flipping its group sums tests symmetry around zero, not equality of action means; a small sign-flip $p$ value must not be read as independent evidence that intervention helps. Appendix~\ref{app:proof} gives the derivation and tie cases.

Figure~\ref{fig:design} shows the information boundary. Frozen policies use the prefix only. The outcome-informed selector exists solely to quantify same-draw optimism; it is not a deployable method or a training label for the confirmation policies. For a policy fixed independently of confirmation outcomes, scoring its chosen action on one fresh draw is unbiased for its conditional continuation value; the second draw improves precision.
...
\qquad a\in\{1,2,3\}.
\end{equation}
By linearity, $\bar D_{ia}=\bar Y_{ia}-\bar Y_{i0}$; the direct learner models this empirical difference rather than the four outcome means. Its continuation score is fixed at zero. Thus the two learners differ in the response target while retaining the same action menu and predictor class. The comparison does not assume that direct effect regression is uniformly preferable; such an ordering is absent even in standard heterogeneous-effect settings \citep{kunzel2019metalearners}.

Both learners receive public information available at the fork: an encoded environment indicator (\texttt{env\_scienceworld}), checkpoint index, counts of invalid or inadmissible actions, reward and usage summaries, action repetition signals, recent observations and actions, and recent prefix text. Hidden state hashes and reversibility metadata are excluded. Numeric fields are vectorized directly. Recent text uses a 512-term TF--IDF vocabulary fitted within each training fold and at most 16 truncated-SVD components.

Model selection uses four shuffled GroupKFold splits by task ID with seed 260908; draws from one task stay in the same fold, while floorplan identity is not used to form folds. Each candidate has 200 trees, minimum leaf size 5 or 10, and a continuation margin in $\{0.00,0.05,0.10\}$. Vocabulary and projection fitting occur inside each fold. Out-of-fold utility selects the configuration, and lower intervention rate breaks utility ties. The final forests are refit on all admitted development tasks. Source and target hashes, fold identities, feature schemas, library versions, and every candidate score are stored in the model manifests.

\subsection{Fallbacks and the comparator}

The policy set contains five distinct decisions at confirmation time. \textsc{Continue} always chooses $a=0$. \textsc{Best fixed} chooses the development-best arm, which is replanning ($a=2$) in both environments. \textsc{Direct} uses the signed-benefit learner. \textsc{Arm outcome} uses the two-draw outcome forest and isolates response parameterization. \textsc{Matched comparator} is the strongest runnable member of the existing arm-outcome family by development out-of-fold utility. It is \textsc{Single-1}, an otherwise matched forest fitted to one predesignated outcome draw. The primary \textsc{Direct}-versus-\textsc{Matched comparator} contrast asks whether the new policy beats the strongest development-selected runnable outcome learner. Because \textsc{Single-1} uses one outcome draw while \textsc{Direct} uses the mean of two, that contrast does not isolate target parameterization. The secondary \textsc{Direct}-versus-\textsc{Arm outcome} contrast does: both learners use the same two-draw task records, features, folds, capacity, and action menu.
...
Folds and capacity & 4 task-ID folds; 200 trees & 4 task-ID folds; 200 trees \\
...
ALFWorld episodes have a 50-action limit and checkpoints at step 4 or 8 \citep{shridhar2021alfworld}. ScienceWorld episodes have a 100-action limit and checkpoints at step 8 or 16 \citep{wang2022scienceworld}. A hash of each task identity assigns the checkpoint before generation. This exogenous schedule fixes the decision point without conditioning on a detected error. It tests one-shot intervention during ordinary execution, which differs from a monitor that fires only after an anomaly; the design cannot determine how an event-triggered monitor would trade recoveries against disruptions. Action 3 restores the state before the last executed action and reduces the suffix limit by one. The other arms restore the checkpoint state. At a checkpoint after $k$ actions with episode limit $B$, arm 3 restores step $k-1$ under limit $B-1$, leaving $(B-1)-(k-1)=B-k$ new actor decisions, the same number as the other arms. The intervention message is inserted once at the restored step and remains in history; no controller sees future state or outcome fields.
...
The ALFWorld primary family contains three quantities: mean same-draw optimism $\mathbb E[G]$, \textsc{Direct} minus \textsc{Continue}, and \textsc{Direct} minus \textsc{Matched comparator}. For each, we average the paired task contrast over all \AlfPlanned{} planned tasks. Early-terminal tasks contribute zero. The preregistered analysis uses a two-sided sign-flip test over floorplan-group sums with 50,000 Monte Carlo assignments and Holm correction across the three quantities at familywise $\alpha=0.05$. We retain these calculations in the audit. For $G$, however, pathwise nonnegativity makes the symmetric sign-flip null scientifically narrow, so its $p$ value is not used to infer action heterogeneity or policy benefit. A positive controller contrast with an adjusted $p$ value above 0.05 is not called an improvement.

Uncertainty plots show 10,000-resample percentile intervals at two levels. The task bootstrap resamples task blocks. The grouped bootstrap resamples complete floorplan groups or ScienceWorld task families and retains all tasks in a sampled group. These intervals describe marginal sampling variability. The preregistered sign-flip and Holm procedure governs the controller contrasts; the magnitude and group interval are the main empirical summaries for $G$. The direct-versus-arm-outcome contrast is the controlled target-parameterization comparison and remains secondary, without a multiplicity-adjusted claim. The evaluator also reports direct-versus-best-fixed, the development-selected policy, intervention rate, action counts, recoveries, disruptions, requests, tokens, and client wall time. Every ScienceWorld comparison is retained regardless of sign.
...
Two current methods are closest to the decision studied here. Calibration Is Not Control fits random-forest intervention policies from same-prefix branches, but its main controller evaluation uses one realized outcome per action; repeated outcomes are reported only for three actions on 24 shallow prefixes \citep{zhang2026calibration}. COTA trains a 0.5B pairwise advisor to rank concrete candidate actions and reports 82.84--90.30\% ALFWorld success with Qwen3-8B \citep{jiang2026cota}. Its 30-step actor, environment-action candidates, repeated intervention schedule, and offline H200 budget differ from our one-shot four-message design. Neither headline score is therefore a matched estimate of our controller contrast. Our strongest matched comparator is the development-selected single-draw outcome forest run with the same actor revision, prefix history, action support, decision budget, features, and confirmation outcomes.

The technical comparison isolates two method choices. Outcome modeling estimates all four response functions; direct signed-benefit modeling estimates three effects relative to continuation, as in the response-versus-effect distinction studied by \citet{kunzel2019metalearners}. Cross-draw scoring then separates maximization from evaluation, following the same statistical principle as Double Q-learning \citep{vanhasselt2010double}. Unlike observational treatment-effect estimation, every action here is executed from a serialized simulator state. The estimand is therefore the utility of the frozen action menu, not a general causal effect or an unmatched benchmark rank.
...
The direct and arm-outcome forests are controlled small-data baselines. Their null contrasts do not bound the performance of neural trace-level advisors, event-triggered monitors, search-based repair, or interventions outside this fixed menu. A null direct-versus-outcome comparison may reflect target parameterization, limited features, development shift, or forest capacity. A favorable result would remain local to the frozen action menu. The present checkpoint and restore protocol has not been validated for nondeterministic external tools or multi-turn state drift. Direct chose rollback on no ALFWorld confirmation prefix, but these data do not identify whether features, training sample size, forest capacity, or the action itself caused that choice. The group bootstrap also inherits the benchmark's floorplan and task-family grouping choices. More environments, actor families, intervention times, and continuation draws are required before recommending a deployment policy.
...
Appendix~\ref{app:actions} specifies the intervention prompts and information contract; Appendix~\ref{app:controller} gives the controller grids; Appendix~\ref{app:lineage} documents task construction and frozen prediction lineage; and Appendix~\ref{app:repro} gives software, inference settings, and exact commands. The anonymous supplement includes the frozen configurations and predictions, raw result hashes, failure records, evaluator, figure builder, environment lock, and a script that verifies and regenerates every reported number. The main repository commit and every derived artifact are linked by SHA-256 manifests.
