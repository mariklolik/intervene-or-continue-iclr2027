...
def variance_sentence(measurement: dict) -> str:
    profile = measurement.get("variance_profile")
...
def write_scienceworld(report: dict, out: Path) -> None:
    ...
    sw_gap = sw["same_draw_selection_optimism"]
    sw_dc = sw["contrasts"]["DIRECT_ADVANTAGE_vs_CONTINUE"]
    text = f"""ScienceWorld provides a prespecified boundary rather than a second opportunity to claim success. In the stored eligible ScienceWorld panel, 39 of 51 tasks fail in all eight action/draw cells (76.5\\%); these tasks necessarily contribute zero to the diagnostic gap. The smaller domain gap can therefore reflect a success floor rather than more stable continuation outcomes. Its same-draw gap is {pct(sw_gap['mean'])} percentage points ({interval(sw_gap, 'group_bootstrap_95')} by task-family bootstrap), and Direct minus Continue is {pct(sw_dc['mean'])} percentage points ({interval(sw_dc, 'group_bootstrap_95')}). Direct replans on {sw['policies']['DIRECT_ADVANTAGE']['action_counts_eligible'][2]:.0f} of {sw['eligible_tasks']} eligible prefixes but changes no observed binary outcome on either draw. The empirical interval collapses to zero because every observed task contrast is zero; it does not establish an exactly zero population effect. All {sw['planned_tasks']} tasks, {sw['groups']} families, six policies, costs, recoveries, and disruptions appear in Table~\\ref{{tab:scienceworld-contrasts}} and the released result JSON. Differences in direction across environments narrow the empirical scope instead of motivating post hoc domain selection.
"""
    out.write_text(text)
...
\\begin{{table}}[ht]
\\caption{{All ScienceWorld contrasts, retained as unadjusted boundary analyses. Values are percentage points. Identical rows are not typesetting errors: on this panel the arm-outcome forest selected arm 2 on every eligible prefix and therefore coincides with \\textsc{{Best fixed}}, and development selection chose \\textsc{{Single-1}}, so \\textsc{{Safe selected}} coincides with \\textsc{{Matched}}.}}
...
\\begin{{table}}[ht]
\\caption{{Mean selected-suffix cost per planned task. Structural early terminations have zero suffix cost. Policies that select the same action on every eligible prefix share a row by construction.}}
\\label{{tab:costs}}
\\centering
\\small
\\begin{{tabular}}{{@{{}}llrrrr@{{}}}}
