...
def pct(value: float, digits: int = 2) -> str:
    number = f"{100 * value:.{digits}f}"
    return f"\\ensuremath{{{number}}}" if value < 0 else number
...
def write_abstract(report: dict, out: Path) -> None:
    ...
    text = (
        ...
        f"{pct(direct_continue['mean'])} percentage points versus continuation and {pct(direct_matched['mean'])} percentage points versus the strongest runnable matched controller. "
...
\\caption{{Group-level measurement and policy effects. Each point is a complete floorplan or task-family group; area is proportional to its task count. Vertical position is same-draw optimism and horizontal position is Direct minus Continue; every ScienceWorld group lies on the horizontal zero-effect line. Dashed lines mark zero.}}
\\label{{fig:groups}}
\\end{{figure}}

ScienceWorld provides a prespecified boundary rather than a second opportunity to claim success. Its same-draw gap is {pct(sw_gap['mean'])} percentage points ({interval(sw_gap, 'group_bootstrap_95')} by task-family bootstrap), and Direct minus Continue is {pct(sw_dc['mean'])} percentage points ({interval(sw_dc, 'group_bootstrap_95')}). All {sw['planned_tasks']} tasks, {sw['groups']} families, six policies, costs, recoveries, and disruptions appear in Table~\\ref{{tab:scienceworld-contrasts}} and the released result JSON. Differences in direction across environments narrow the empirical scope instead of motivating post hoc domain selection. In the stored eligible ScienceWorld panel, 39 of 51 tasks fail under all eight action/draw cells, so their diagnostic gap is necessarily zero. The smaller domain gap can therefore reflect a success floor rather than more stable continuation outcomes. Direct replans on six of the 51 eligible prefixes but changes no observed binary outcome on either continuation draw. Thus the empirical Direct-minus-Continue interval collapses to [0, 0]; this zero sample variation does not establish an exactly zero population effect. In ScienceWorld, Arm outcome selects replan on every eligible prefix and so matches Best fixed exactly; the frozen Safe selected policy is the Matched/Single-1 controller, so those rows also match.
"""
    out.write_text(text)
...
def write_conclusion(report: dict, out: Path) -> None:
    ...
    dm = alf["contrasts"]["DIRECT_ADVANTAGE_vs_MATCHED_COMPARATOR"]
    text = (
        f"On the frozen ALFWorld panel, independent continuation evaluation measures a {pct(gap['mean'])} percentage-point gap relative to same-draw maximization. "
        f"The matched direct controller changes success by {pct(dc['mean'])} percentage points versus continuation and {pct(dm['mean'])} percentage points versus the strongest runnable comparator. "
