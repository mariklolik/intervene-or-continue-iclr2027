...
def write_results(report: dict, out: Path) -> None:
    ...
    text = f"""Relative to continuation, direct signed-benefit prediction changes success by {pct(dc['mean'])} percentage points (group-bootstrap 95\\% interval {interval(dc, 'group_bootstrap_95')}; group sign-flip ${p_claim(dc['group_sign_flip_p'])}$; Holm-adjusted ${p_claim(dc['holm_p_primary_family'])}$), and relative to the matched arm-outcome comparator by {pct(dm['mean'])} points ({interval(dm, 'group_bootstrap_95')}; adjusted ${p_claim(dm['holm_p_primary_family'])}$); the two controllers reach {pct(direct['planned_mean'])}\\% and {pct(matched['planned_mean'])}\\%. The denominator contains all {alf['planned_tasks']} planned tasks: {alf['eligible_tasks']} reached the frozen checkpoint and {alf['early_terminal_tasks']} ended earlier, and Table~\\ref{{tab:confirmation-policies}} reports the full census. Neither primary controller comparison reaches significance at the unadjusted level, so neither is eliminated by the multiplicity correction alone. The development-selected safe policy changes success by {pct(safe['mean'])} points relative to continuation. These tests answer the frozen comparisons; a favorable unadjusted subset is not substituted for them.
