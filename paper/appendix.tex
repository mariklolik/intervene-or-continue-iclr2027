...
Two consequences follow for inference. First, a sign-flip permutation null is degenerate for $G$: non-negativity makes every group sum non-negative, and the observed statistic is the algebraic maximum over sign vectors. Rejection therefore concerns an all-zero $G$ distribution, which can fail even if all actions have the same conditional mean. The first confirmation retains the preregistered $G$ test in its audit family, but we do not use that $p$ value to infer action effects. The second confirmation treats $G$ as an estimate outside its tested family. Second, the mechanical part of $G$ can be quantified directly. Permuting the recorded outcomes within a task across all action and draw cells holds the number of successes fixed, preserves the continuation noise level, and destroys any action structure. The mean of $G$ under this within-task permutation is the noise floor that a homogeneous menu would produce, and the observed $G$ is read against it. With $R>2$ draws we average $G$ over ordered draw pairs, which reduces to Equation~\ref{eq:gap} at $R=2$.
...
All arms begin from the same task and baseline checkpoint with the same actor weights, decoding parameters, and maximum decision budget. A rollback arm then restores the public history and serialized state at step $t-k$ after reverting $k$ actions, regardless of domain-level physical reversibility. Under episode limit $L$, its suffix limit is $L-k$, leaving $(L-k)-(t-k)=L-t$ possible new actor decisions, the same as for an arm that does not roll back. Counting the discarded baseline actions, every arm has the same maximum total budget of $L$ decisions; early termination may use fewer. The controller receives only fields computed from the prefix. Environment state hashes, future observations, suffix outcomes, rollback metadata, and task-selection provenance are excluded. Arm 3 has no special access to a future legal-action sequence. The intervention decision occurs once per episode.

\section{Controller construction and development selection}
\label{app:controller}

The 50 numeric features include action-class fractions; counts and rates of admissible, repeated, invalid, and failed actions; reward summaries; observation and prompt lengths; token usage; action and observation novelty; vocabulary growth; step counts; and the \texttt{env\_scienceworld} indicator. The indicator is constant within each environment-specific forest; separate forests are fitted by actor and environment. The features \texttt{steps\_so\_far} and \texttt{log\_steps\_so\_far} encode checkpoint progress. No categorical task identifier is encoded. The text field concatenates the recent public observations and actions. Word TF--IDF uses a vocabulary of at most 512 terms, and the resulting representation is reduced to at most 16 truncated-SVD components. All text fitting occurs inside each task-ID fold during development selection. The final transform is refit on all admitted development tasks only.
...
\input{generated/scienceworld.tex}

\input{generated/results_floats.tex}

\input{generated/analysis_floats.tex}

\input{generated/appendix_results.tex}

\input{generated/r2_appendix.tex}
...
\input{generated/r2_temperature.tex}
