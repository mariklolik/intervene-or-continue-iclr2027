...
def stratum_differences(rows, leaf, margin, agreement, candidates):
    y = np.asarray([row["Y"] for row in rows], dtype=float)
    targets, utility = cross_draw.draw_targets(y), y.mean(axis=1)
    predicted, evidence = cross_draw.fold_predictions(rows, targets, leaf, task_folds(rows))
